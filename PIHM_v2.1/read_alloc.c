...
void
read_alloc(char *filename, Model_Data DS, Control_Data * CS)
{
	int             i, j;
	int             tempindex;
	...
	for (i = 0; i < DS->NumSoil; i++) {
		...
		/*
		 * Note: Soil KsatH and macKsatH is not used in model
	...
	if (DS->Num2BC > 0) {
		/*
		 * For elements with Neumann (non-natural) Boundary
		 * Conditions
		 */
	...
	for (i = 0; i < CS->NumSteps + 1; i++) {
		if (i == 0) {
			...
			//? ? CS->Tout[i] = CS->Tout[i - 1] + pow(CS->a, i) * CS->b * 60;
	...
	/* THIS IS FROM XUAN FOR SHALEHILLS ONLY -- NEXT THREE LINE?? */
	//??for (i = 0; i < DS->NumSoil; i++) {
	//??	DS->Soil[i].ThetaS = DS->Soil[i].ThetaS * CS->Cal.Porosity;
	//??}
