...
initialize(char *filename, Model_Data DS, Control_Data * CS, N_Vector CV_Y)
{
	...
	for (i = 0; i < DS->NumRiv; i++) {
		...
		for (j = 0; j < 3; j++) {
			/*
			 * Note: Strategy to use BC < -4 for river
			 * identification
			 */
		...
		/*
		 * Note: Ideally this data should be read from the
		 * decomposition itself
		 */
		/*
		 * but it is not supported right now in PIHMgis (Bhatt, G and
		 * Kumar, M; 2007)
		 */
	...
	if (CS->init_type == 0) {
		...
		for (i = 0; i < DS->NumRiv; i++) {
			...
			/*
			 * Note once the element beneath river is
			 * incorporated in decomposition and .mesh file,
			 * initialization should be perfomed based on the
			 * location data instead of average of neighbor
			 * properties
			 */
	...
	/* data initialization mode */
		...
		if (DS->UnsatMode == 2) {
			...
			for (i = 0; i < DS->NumRiv; i++) {
				...
				/*
				 * Note once the element beneath river is
				 * incorporated in decomposition and .mesh
				 * file, initialization should be perfomed
				 * based on the location data instead of
				 * average of neighbor properties
				 */
