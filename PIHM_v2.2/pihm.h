...
typedef struct river_shape_type {
	...
	int             interpOrd;	/* Interpolation order for river
					 * shape: Order =1 (rectangle),
					 * 2(triangle), 3(quadratic) and
					 * 4(cubic) */
	realtype        coeff;	/* Coefficient c in D = c*pow(B/2,interpOrd) */
...
typedef struct control_data_structure {
	...
	int             snowD;	/* File boolean, Choose 1 if your want to
				 * print snow Depth */
	int             rivStg;	/* File boolean, Choose 1 if your want to
				 * print river Stage */
	...
	int             et[3];	/* File boolean, Choose 1 if your want to
				 ...
				 * components */
	int             rivFlx[10];	/* File boolean, Choose 1 if your
